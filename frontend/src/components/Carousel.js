...
function Carousel({image}) {
  ...
  return (
    <div className="group flex flex-row items-center justify-center w-full relative">
        <button className='absolute h-full z-10 pl-2 left-1 opacity-0 hover:opacity-100 duration-200' onClick={previousImage}>
        ...
        <img
            ...
            title={""}
            />
        <button className='absolute h-full z-10 right-1 pr-2 opacity-0 hover:opacity-100 duration-300' onClick={nextImage}>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
